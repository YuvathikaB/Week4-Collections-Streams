...
public class ClaimsAnalysis {
    public static void main(String[] args) {
        ...
        totalClaims.forEach((policy, total) -> {
            double avg = averageClaims.get(policy);
            System.out.printf("Policy: %s, Total: $%.2f, Average: $%.2f%n", policy, total, avg);
        });
        ...
        totalClaims.entrySet().stream()
                ...
                .limit(3)
                .forEach(entry -> {
                    System.out.printf("Policy: %s, Total Claim Amount: $%.2f%n", entry.getKey(), entry.getValue());
                });
    }
}
